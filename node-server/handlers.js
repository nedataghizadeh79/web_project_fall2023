...
const pfp_storage = multer.diskStorage({
  ...
  filename: function(req, file, cb) {
    cb(null, 'pfp-' + req.USER_ID + path.extname(file.originalname));
  }
});
...
export const upload_pfp = async function (req, res) {
  try {
    req.USER_ID = req.body.USER_ID;
    let upload = multer({ storage: pfp_storage }).single('file');
...
export const download_pfp = async function (req, res){
  try{
    const options = {
      root: path.join("./")
    };
    return res.sendFile("./media/" + req.params.id, options);
  }catch (error){
    responseUtils.server_error(error, res);
  }
}
...
export const ta_history = async function (req, res){
  try{
    const {id} = req.body;
    const history = await model.TaHistory.findAll({where: {id: id}});
    res.send(history)
